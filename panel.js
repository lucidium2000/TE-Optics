...
(function () {
  ...
  // TE's path-vis graph gives only geonameId + a human "City, Region, Country"
  // string (no lat/lng); the world map resolves those client-side. CSP blocks
  // external geocoders, so we resolve locally. The LIVE TEST target is always
  // 8.8.8.8, whose final hop lands in a Google anycast metro — this table covers
  // the realistic set, keyed by exact geonameId first, then by city name.
  const GEONAME_COORDS = {
    '4180439': [33.7490, -84.3880],   // Atlanta, GA
    '4174757': [27.9475, -82.4584],   // Tampa, FL
    '4299276': [38.2542, -85.7594],   // Louisville, KY
    '4487042': [35.7721, -78.6386],   // Raleigh, NC
    '5128581': [40.7128, -74.0060],   // New York, NY
    '5375480': [37.3861, -122.0839],  // Mountain View, CA
  };
  const METRO_COORDS = {
  ...
  /**
   * Resolve a TE geonameId / "City, Region, Country" string → {lat,lng}.
   * Returns null when the geo is only country-level (e.g. "US", geonameId
   * 6252001): TE couldn't pin the anycast PoP, so we must NOT collapse those
   * agents onto the country centroid (that was the "generic US" marker).
   */
  function liveTestResolveDestGeo(geonameId, locationName) {
    const gid = geonameId != null ? String(geonameId) : '';
    // 1) Exact geonameId (precise Google PoP coords we've curated).
    if (gid && GEONAME_COORDS[gid]) { const c = GEONAME_COORDS[gid]; return { lat: c[0], lng: c[1] }; }
    if (locationName) {
      const parts = String(locationName).split(',').map((s) => s.trim()).filter(Boolean);
      // 2) Curated Google-metro table (precise city centroids).
      ...
      if (parts.length >= 2) {
        // 3) Broad gazetteer — but reject a country-centroid match, since that's
        //    the misleading generic marker, not a real PoP city.
        try {
          const g = epGeocode(locationName);
          if (g && g.precision !== 'country' && Number.isFinite(g.lat) && Number.isFinite(g.lng)) {
            return { lat: g.lat, lng: g.lng };
          }
        } catch (_) { /* */ }
      }
    }
    return null;   // unresolved / country-level only → no path drawn
  }
  ...
  async function liveTestFetchPathVisDest(testId, agentIds) {
    ...
    for (const id of agentIds) {
      ...
      if (!loc || (loc.geonameId == null && !loc.locationName)) continue;
      const geo = liveTestResolveDestGeo(loc.geonameId, loc.locationName);
      if (geo && Number.isFinite(geo.lat) && Number.isFinite(geo.lng)) {
        ...
        // TE only resolved this agent's 8.8.8.8 hop to country level (no PoP city);
        // omit the path rather than collapse it onto a generic country centroid.
        liveTestGeoSkip.add(key);
        log(`LIVE TEST: agent ${key} → 8.8.8.8 geo "${tepEscapeHtmlText(loc.locationName || ('geoname ' + loc.geonameId))}" is country-level only — path omitted (no PoP city)`, 'tep-log-info');
      }
    }
    return stored;
  }
