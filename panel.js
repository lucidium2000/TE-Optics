...
(function () {
      ...
      <div class="tep-view-panel" id="tep-panel-tags">
        <div class="tep-create-block">
          ...
          <div class="tep-manage-expand tep-create-expand" id="tep-tags-expand">
            ...
            <p class="tep-dash-hint" style="margin:10px 0 0;font-size:11px;color:#64748b;line-height:1.45;">Restore creates the selected tag definitions (key, value, color, object type) as new tags. Object assignments are not re-applied — assignment ids reference the source account&rsquo;s objects.</p>
            <div class="tep-dash-row" style="margin-top:14px;">
  ...
  /** Best-effort human label for an entry in a bulk-create `errors[]` response. */
  function bulkErrorTagLabel(e) {
    if (!e || typeof e !== 'object') return 'tag';
    const t = e.tag;
    if (t && typeof t === 'object') {
      if (t.key != null || t.value != null) return formatTagRestoreLabel(t).name;
      // Schema wraps as additionalProperties map → grab the first tag-like value.
      const vals = Object.values(t).filter((v) => v && typeof v === 'object');
      if (vals.length) return formatTagRestoreLabel(vals[0]).name;
    }
    return 'tag';
  }
  ...
  async function restoreSelectedTags() {
    ...
    if (!confirm(`Restore ${arr.length} tag definition(s) to this account? Object assignments are not re-applied. Continue?`)) return;
    const dismiss = toastProcessing(`Restoring ${arr.length} tag(s)…`);
    try {
      // The internal tags service rejects POST on the collection root (405);
      // creation goes through the bulk endpoint with a { tags: [...] } body.
      const bodyTags = arr.map(cleanTagForRestore);
      const url = `${TAGS_API_PATH}/bulk`;
      log(`Tags: POST ${url} (${bodyTags.length} tag(s))`, 'tep-log-info');
      const resp = await ajax(url, { method: 'POST', body: JSON.stringify({ tags: bodyTags }) });
      const text = await resp.text().catch(() => '');
      // 207 Multi-Status is the documented success code for bulk create.
      if (!resp.ok && resp.status !== 207) {
        log(`Tags restore failed ${resp.status}: ${text.slice(0, 240)}`, 'tep-log-err');
        if (isLikelyPermissionDenied(resp.status, text)) setStatus(TEP_STATUS_SUPERPOWERS, 'err');
        toast(`Restore failed: ${resp.status}`, 'err');
        return;
      }
      let data = null;
      try { data = JSON.parse(text); } catch (_) { /* non-JSON body */ }
      const created = data && Array.isArray(data.tags) ? data.tags : null;
      const errors = data && Array.isArray(data.errors) ? data.errors : [];
      let ok;
      let fail;
      if (created || errors.length) {
        ok = created ? created.length : (arr.length - errors.length);
        fail = errors.length;
        for (const e of errors) {
          const msg = [e && e.responseCode, e && e.message].filter(Boolean).join(' ').trim() || 'error';
          log(`Tag restore: "${bulkErrorTagLabel(e)}" failed: ${msg}`, 'tep-log-err');
        }
      } else {
        // No structured body on a 2xx/207 → assume all created.
        ok = arr.length;
        fail = 0;
      }
      log(`Tags restore complete: ${ok} created, ${fail} failed`, fail ? 'tep-log-err' : 'tep-log-ok');
      toast(`Restored ${ok} tag(s)${fail ? `, ${fail} failed` : ''}`, fail ? 'err' : 'ok');
    } catch (e) {
      log(`Tags restore error: ${e.message}`, 'tep-log-err');
  ...
  function getThousandEysMilliSumPerRound(nC, nE, t) {
    ...
    if (isThousandEysFixed5MilliTestType(t)) {
      // Fixed 5 (cloud) / 2.5 (enterprise) milli; DNS Server scales by server count.
      const factor = isDnsServerTest(t) ? getDnsServerCount(t) : 1;
  ...
  function epAgentTipRow(a, idx) {
    ...
    return `<div class="tep-map-tip-agent" data-idx="${idx}" role="link" tabindex="0" title="Open this agent in ThousandEyes">
      <div class="tep-map-tip-name">${tepEscapeHtmlText(a.name)}</div>
      ${users}
  ...
  function renderEndpointAgentsMap(listEl, agents, countEl) {
    ...
    tip.addEventListener('click', (e) => { openTipAgent(e.target.closest('.tep-map-tip-agent')); });
  ...
  function buildDashboardMapAgents() {
    ...
    for (const a of agents) {
      ...
      list.push({
        ...
        lat, lng, health: tepEnterpriseHealth(a.status), url: null,
      });
    }
    for (const a of allEndpointAgents) {
      ...
      list.push({
        ...
        lat: g.lat, lng: g.lng, health: tepEndpointHealth(a.lastSeenMs),
  ...
  function tepDashTipRow(it, idx) {
    const clickable = it.kind === 'endpoint' && it.url && it.url !== '#';
    ...
    const attrs = clickable ? ` data-idx="${idx}" role="link" tabindex="0" title="Open this agent in ThousandEyes"` : '';
    return `<div class="${cls}"${attrs}>
      <div class="tep-map-tip-name">${tepEscapeHtmlText(it.name)}${badge}</div>
  ...
  function tepDashTooltipHtml(cluster) {
    ...
    if (n > cap) body += `<div class="tep-map-tip-more">+${n - cap} more…</div>`;
    body = `<div class="tep-map-tip-body">${body}</div>`;
    const hasEp = cluster.items.some((it) => it.kind === 'endpoint' && it.url && it.url !== '#');
    const foot = hasEp ? 'Click an endpoint agent to open it' : 'Enterprise agents — hover for details';
    return `<div class="tep-map-tip-head">${head}</div>${body}<div class="tep-map-tip-foot">${foot}</div>`;
  }
  ...
  function tepWidgetCard(title, inner, extraClass) {
    return `<div class="tep-dash-widget ${extraClass || ''}"><div class="tep-dash-widget-title">${title}</div>${inner}</div>`;
  }
  ...
  function renderDashWidgets(container) {
    ...
    container.innerHTML =
      ...
      + tepWidgetCard('Alerts', '<div id="tep-w-alerts">' + tepWidgetPending() + '</div>', 'tep-dash-widget--alert')
  ...
  async function fillDashWidgetsAsync() {
    ...
    if (alertsEl) {
      ...
      alertsEl.innerHTML = a
        ? `<div class="tep-dash-widget-main">${a.active}<small> active</small></div><div class="tep-dash-widget-sub">${a.recent} recent</div>`
        : tepWidgetPending();
    }
  ...
  // Providers pending TE in-app API discovery (return { active, recent } / { active }).
  async function tepFetchAlertsSummary() { return null; }
