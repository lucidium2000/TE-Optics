  ...
  function renderDashboardAgentMap(hostEl, opts) {
    ...
    function buildFlow() {
      ...
      for (const it of list) {
        ...
        const dp = tepLonLatToPct(dest.lng, dest.lat);
        ...
        if (!destSeen.has(key)) {
          ...
          de.setAttribute('aria-label', '8.8.8.8 (Google DNS)' + (dest.location ? ' — ' + dest.location : ''));
          de.innerHTML = '<span class="tep-livetest-g">' + tepGoogleGIconSvg(16) + '</span>';
          de._destInfo = dest.info || { ip: '8.8.8.8', name: 'dns.google', location: dest.location };
          de._destLoc = dest.location;
