...
(function () {
  ...
  const STYLES = `
    ...
    .tep-livetest-g {
      ...
      background: #fff; color: #4285F4; font-weight: 800; font-size: 15px;
    ...
    .tep-gcard-g {
      ...
      font-family: Arial, Helvetica, sans-serif; box-shadow: 0 0 0 2px #4285F4;
    }
  ...
  liveTestBtn.title = 'Experimental: instant network test to 8.8.8.8 from all online enterprise + endpoint agents';
  ...
  function tepGoogleNodeCardHtml(info, locStr, agents, hasEstimated) {
    ...
    const head = `<div class="tep-gcard-head"><span class="tep-gcard-g">G</span>`
  ...
  function renderDashboardAgentMap(hostEl, opts) {
    ...
    function paintMarker(m) {
      ...
      const isSrc = liveTestRunning && items.some((it) => it.kind === 'enterprise');
    ...
    function buildFlow() {
      ...
      for (const it of list) {
        ...
        if (!destSeen.has(key)) {
          ...
          de.innerHTML = '<span class="tep-livetest-g">G</span>';
  ...
  // ===========================================================================
  // LIVE TEST (experimental) — instant network test to 8.8.8.8 from all online
  // enterprise agents (A2S, TCP SYN + in-session) and all online endpoint agents
  // (endpoint network, AUTO probe). Runs a 1-minute / 5-second progress poll.
  // ===========================================================================
  ...
  let liveTestPathVisLoggedShape = false;   // reset per run; logs the raw path-vis graph shape once
   ...
   */
  async function liveTestFetchPathVisDest(testId, agentIds) {
    if (testId == null || testId === '' || !Array.isArray(agentIds) || !agentIds.length) return 0;
    ...
    if (!liveTestPathVisLoggedShape) {
      liveTestPathVisLoggedShape = true;
      log(`LIVE TEST path-vis: ${nodes.length} node(s), ${destNodes.length} 8.8.8.8 dest node(s)`, 'tep-log-info');
      ...
      log(`LIVE TEST path-vis: ${routesAll.length} route(s), ${destRoutes.length} landing on 8.8.8.8`, 'tep-log-info');
  ...
  async function runLiveTest() {
    ...
    liveTestPathVisLoggedShape = false;
    ...
    // Ensure agent inventories are loaded (endpoint agents still shown on the map,
    // but LIVE TEST only runs the enterprise instant now).
    ...
    toast(`LIVE TEST started — ${results.entOk ? 'enterprise instant running' : 'nothing to run'}`, results.entOk ? 'ok' : 'err');
  }

  /** Per-poll: pull per-agent latency for the enterprise instant test → map labels. */
  async function liveTestPoll(results, startedAt) {
    const elapsed = Math.round((Date.now() - startedAt) / 1000);
    if (!results.entOk || results.entTestId == null) {
      log(`LIVE TEST poll @${elapsed}s: no enterprise instant to poll`, 'tep-log-info');
      return;
    }
    let n = 0;
    ...
    if (n || destN) { liveTestLatencyActive = true; try { refreshLiveTestOverlay(); } catch (_) { /* */ } }
  }
