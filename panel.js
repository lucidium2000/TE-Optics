...
(function () {
   ...
   * `generalFilters`, drop it unless the dimension ID appears as a string
   * reference elsewhere in the dashboard (e.g. `groupCardsBy`, `cards`,
   * `aggregateProperties`, `colProperty`, `rowProperty`, `metric`). Structural
   * dimensions are kept with EMPTY values so TE re-accepts the payload
   * without re-populating defaults.
   *
  ...
  function stripAllWidgetFiltersFromDashboardJson(root) {
    ...
    // Pass 1: collect all string values that appear OUTSIDE filters/generalFilters
    // value containers. Any dimension ID referenced this way is "structural".
    ...
    (function collect(v, insideFilterBlock) {
      ...
      if (v && typeof v === 'object') {
        for (const [k, sub] of Object.entries(v)) {
          collect(sub, insideFilterBlock || k === 'filters' || k === 'generalFilters');
        }
      }
    })(root, false);

    // Pass 2: clear or drop dimensions in every filters/generalFilters object.
    (function walk(v, depth) {
      if (depth > 42 || v === null || typeof v !== 'object') return;
      if (Array.isArray(v)) { for (const x of v) walk(x, depth + 1); return; }
      if (v.filters && typeof v.filters === 'object' && !Array.isArray(v.filters)) {
        for (const dim of Object.keys(v.filters)) {
          if (structural.has(dim)) {
            if (Array.isArray(v.filters[dim])) v.filters[dim] = [];
          } else {
            delete v.filters[dim];
          }
        }
      }
      if (v.generalFilters && typeof v.generalFilters === 'object' && !Array.isArray(v.generalFilters)) {
        for (const dim of Object.keys(v.generalFilters)) {
          if (structural.has(dim)) {
            const entry = v.generalFilters[dim];
            if (entry && typeof entry === 'object' && !Array.isArray(entry)) {
              if (Array.isArray(entry.values)) entry.values = [];
              if (Object.prototype.hasOwnProperty.call(entry, 'value')) entry.value = null;
            }
          } else {
            delete v.generalFilters[dim];
          }
        }
      }
  ...
  function syncDashRestoreAgentUi() {
    ...
    if (note && mode === 'strip') {
      note.textContent = 'Widget filter values (filters and generalFilters) are cleared, and virtual-agent fields are emptied. Dimension structure is preserved so TE accepts the restore.';
    }
  }
