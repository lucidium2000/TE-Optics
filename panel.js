...
(function () {
        ...
        <div class="tep-manage-block" id="tep-ep-block-agents">
          ...
          <div class="tep-create-expand" id="tep-ep-expand-agents" hidden>
            ...
            <div class="tep-manage-toolbar">
              ...
              <select id="tep-ep-agents-license">
  ...
  function formatTagRestoreLabel(t) {
    ...
    const name = key && value ? `${key}: ${value}` : (value || key || '(unnamed tag)');
  ...
  async function restoreSelectedTags() {
    ...
    try {
      for (const src of arr) {
        ...
        try {
          const resp = await ajax(TAGS_API_PATH, { method: 'POST', body: JSON.stringify(body) });
          const text = await resp.text().catch(() => '');
  ...
  function isThousandEysTimeoutMilliTestType(t) {
    ...
    if (isApiTest(t) || isVoipRtpTest(t)) return true;
    const ty = String(t.type == null ? '' : t.type).toLowerCase();
  ...
  function getThousandEysWebTimeoutSecForUnits(t) {
    ...
    if (isVoipRtpTest(t)) {
      // RTP stream bills on the stream duration (default 5s).
      return clampThousandEysTimeoutSec(
        t.duration != null ? t.duration : (t.codecDuration != null ? t.codecDuration : t.callDuration),
        5
      );
    }
  ...
  function populateEndpointAgentLicenseFilter() {
    const sel = $('#tep-ep-agents-license');
    if (!sel) return;
    const prev = sel.value;
    const licenses = Array.from(new Set(allEndpointAgents.map((a) => a.license).filter(Boolean))).sort();
    sel.innerHTML = '<option value="">All licenses</option>'
      + licenses.map((l) => `<option value="${tepEscapeHtmlText(l)}">${tepEscapeHtmlText(l)}</option>`).join('');
  ...
  function sortEndpointAgents(list) {
    const sortEl = $('#tep-ep-agents-sort');
    const mode = sortEl ? sortEl.value : 'last-contact';
  ...
  function buildEndpointAgentCard(agent, showPointer) {
    ...
    const plat = [agent.platform, agent.osVersion].filter(Boolean).join(' ');
    if (plat) meta.push(`<span title="Platform">${tepEscapeHtmlText(plat)}</span>`);
  ...
  function renderEndpointAgents() {
    ...
    const sortEl = $('#tep-ep-agents-sort');
    const groupByTag = !!(sortEl && sortEl.value === 'group-tag');
    if (!groupByTag) {
  ...
  async function loadEndpointAgents() {
    ...
    try {
      ...
      allEndpointAgents = elements.map(normalizeEndpointAgent).filter((a) => a.id);
      const dropped = elements.length - allEndpointAgents.length;
      if (dropped > 0) log(`Endpoint agents: skipped ${dropped} element(s) with no resolvable id`, 'tep-log-info');
      if (elements.length) log(`Agent element keys: ${topLevelKeysLabel(epAgentPickRaw(elements[0]))}`, 'tep-log-info');
      ...
      populateEndpointAgentLicenseFilter();
  ...
  if (isEndpointToolsPage()) {
    ...
    if (epAgentsSort) epAgentsSort.addEventListener('change', () => { renderEndpointAgents(); });
    const epAgentsLicense = root.querySelector('#tep-ep-agents-license');
    if (epAgentsLicense) epAgentsLicense.addEventListener('change', () => { renderEndpointAgents(); });
